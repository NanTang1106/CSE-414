...
public class HW6 {
  ...
  public static Dataset<Row> Q1 (SparkSession spark, String dataFile) {
    ...
    Dataset<Row> leave_seattle =
            spark.sql("SELECT DISTINCT f.destcityname FROM flights f WHERE origincityname = 'Seattle, WA'");

    return leave_seattle;
  }
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> leave_seattle = d.filter(t -> t.get(ORIGIN_CITY_NAME).toString().equals("Seattle, WA"));

    JavaRDD<Row> dest_city = leave_seattle.map(t -> RowFactory.create(t.get(DEST_CITY_NAME))).distinct();
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> no_cancel = d.filter(t -> t.getInt(CANCELLED)==0);

    JavaPairRDD<Tuple2<String, Integer>, Integer> city = no_cancel.mapToPair(t ->
            new Tuple2<>(new Tuple2<String, Integer>(t.get(ORIGIN_CITY_NAME).toString(), t.getInt(MONTH)), 1));

    JavaPairRDD<Tuple2<String, Integer>, Integer> city_count = city.reduceByKey((v1, v2) -> v1 + v2);
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> origin_dest = d.map(t -> RowFactory.create(t.get(ORIGIN_CITY_NAME).toString(),
            t.get(DEST_CITY_NAME).toString())).distinct();

    JavaPairRDD<String, Integer> origin = origin_dest.mapToPair(t -> new Tuple2<>(t.get(0).toString(), 1));

    JavaPairRDD<String, Integer> origin_count = origin.reduceByKey((v1, v2) -> v1 + v2);

    Tuple2<String, Integer> origin_max = origin_count.max(new TupleComparator());

    String max_city = origin_max._1().toString();

    int max_city_flights = (int)d.filter(t -> t.get(ORIGIN_CITY_NAME).toString().equals(max_city)).count();

    Tuple2<String, Integer> city_count = new Tuple2(max_city, max_city_flights);
  ...
  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {
    ...
    JavaRDD<Row> d_no_null = d.filter(t -> t.get(DEP_DELAY) != null);

    JavaPairRDD<String, Tuple2<Integer, Integer>> city_delay = d_no_null.mapToPair(t ->
            new Tuple2<>(t.get(ORIGIN_CITY_NAME).toString(), new Tuple2<Integer, Integer>(t.getInt(DEP_DELAY), 1)));

    JavaPairRDD<String, Tuple2<Integer, Integer>> delay_sum = city_delay.reduceByKey((v1, v2) ->
            new Tuple2<Integer, Integer>(v1._1() + v2._1(), v1._2() + v2._2()));

    JavaPairRDD<String, Double> percent_delay = delay_sum.mapToPair(t ->
            new Tuple2<String, Double>(t._1(), 1.0 * (t._2()._1()) / t._2()._2()));
